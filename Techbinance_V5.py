...
class BinanceFuturesBot:
    def __init__(self, env_path: str, symbol: str = "BTCUSDT", leverage: int = 10, trailing_stop_pct: float = 15.0):
        ...
        self.trailing_stop_pct = trailing_stop_pct  # 1차 매도 후 하락률 기준 (예: 15%)
        self.active_trades = []   # 개별 거래 건별 익절/손절 추적
        # 기준봉 탐색 관련 변수
        self.buy_signal = None           # 기준봉 감지 시 기준 윌리엄 %R 값 저장 (1봉전 값)
        self.baseline_close = None       # 기준봉 완성 시 0봉전 종가 값 저장
    ...
    def safe_buy(self, amount_usdt: float):
        try:
            ...
            self.log(f"🛒 매수 주문 시도: {quantity} {coin} (약 {amount_usdt} USDT)")
            with self.trade_lock:
            ...
            for i in range(ORDER_POLLING_MAX_ATTEMPTS):
                ...
                if status == ORDER_STATUS_FILLED:
                    ...
                    self.log(f"✅ 매수 주문 체결: {executed_qty} {coin} at {avg_price} USDT")
                    return {
                        "order_id": order_id,
                        "price": avg_price,
    ...
    def safe_sell(self, sell_quantity: float):
        try:
            coin = self.symbol.replace("USDT", "")
            self.log(f"🛒 매도 주문 시도: {sell_quantity} {coin}")
            with self.trade_lock:
                order = self.client.futures_create_order(
                    symbol=self.symbol,
                    side=SIDE_SELL,
            ...
            for i in range(ORDER_POLLING_MAX_ATTEMPTS):
                ...
                if status == ORDER_STATUS_FILLED:
                    ...
                    self.log(f"✅ 매도 주문 체결: {executed_qty} {coin} at {avg_price} USDT")
                    return {
                        "order_id": order_id,
                        "price": avg_price,
                        "quantity": executed_qty,
    ...
    def baseline_detection_loop(self):
        ...
        [수정사항]
          ...
          - 새로운 완성 캔들이 없더라도, 마지막 완성된 캔들에 대해 조건이 충족되었고 buy_signal이 아직 없으면 재검증하여 기준봉을 확정합니다.
        """
        while self.running:
            df = self.get_data()
            if df is not None and len(df) >= 3:
                ...
                if (self.last_baseline_candle_time is None or 
                    ...
                    if should_buy:
                        self.buy_signal = baseline  # 기준선1 : 1봉전 윌리엄 %R 값
                        self.baseline_close = df["close"].iloc[-1]  # 기준선2 : 0봉전 종가
                        self.log(f"⚡ 매수 조건 감지! 기준선1: {baseline:.2f}, 기준선2(종가): {self.baseline_close:.2f} (캔들 종료: {latest_candle_time.strftime('%Y-%m-%d %H:%M:%S')})")
    ...
    def execute_trade(self):
        """
        - 상태 업데이트, 포지션 관리, GPT 보고서 갱신 등은 계속 진행됩니다.
        - baseline_detection_loop에서 설정된 self.buy_signal가 있으면,
          최신 완성 캔들 데이터(df)를 기반으로 재검증 후 매수 주문(safe_buy)을 실행하여 거래를 생성합니다.
        """
        self.running = True
        self._init_daily_stats()
        ...
        while self.running:
            try:
                ...
                if self.buy_signal is not None:
                    ...
                    # 추가 조건: 최신 캔들의 종가가 바로 이전 캔들 대비 0.3% 이상 하락한 경우
                    previous_close = df["close"].iloc[-2]
                    current_close = df["close"].iloc[-1]
                    conditionA = (current_william <= self.buy_signal or current_william <= -90)
                    conditionB = (current_close <= previous_close * 0.997)
                        ...
                        if buy_amount < 10:
                            ...
                            if buy_order:
                                ...
                                with self.trade_lock:
                                    ...
                                    self._save_trades()
                                coin = self.symbol.replace("USDT", "")
                                self.log(f"✅ 새로운 매수 포지션 생성: {buy_order['quantity']} {coin} at {buy_order['price']} USDT")
                                # 매수 후 기준선 초기화
                                self.buy_signal = None
                                self.baseline_close = None
    ...
    def check_conditions(self, df):
        ...
        조건이 충족되면 True와 기준값(1봉전 윌리엄 %R 값)을 반환합니다.
        """
        if len(df) < 3:
            return False, None
        rsi_2 = df["rsi"].iloc[-3]
            ...
            for trade in self.active_trades:
                ...
                if trade.get("sell_stage", 0) == 0 and current_price >= entry_price * first_target:
                    ...
                    if sell_order:
                        ...
                        self.log(f"💰 {self.first_target_profit_pct}% 목표 달성: 절반 매도, 남은 수량: {trade['remaining_quantity']} {coin}")
                # 2차 매도 조건 (고정 목표)
                    ...
                    if sell_order:
                        ...
                        self.log(f"💰 {self.second_target_profit_pct}% 목표 달성: 전량 매도, 거래 종료")
                        self.trade_history.append(trade)
                        self._save_trade_history()
                        self._update_daily_stats(trade["profit_usdt"])
                # [추가] 트레일링 스탑 조건: 1차 매도 후, 가격이 첫 매도 가격 대비 trailing_stop_pct 하락하면 전량 매도
                if trade.get("sell_stage", 0) == 1:
                    first_sell_price = trade["first_sell"]["price"]
                    stop_price = first_sell_price * (1 - self.trailing_stop_pct / 100)
                    if current_price <= stop_price:
                        ...
                        if sell_order:
                            ...
                            trade["profit_pct"] = (sell_order["price"] - entry_price) / entry_price * 100 * self.leverage
                            trade["profit_usdt"] = (sell_order["price"] - entry_price) * trade["entry_quantity"]
                            self.log(f"💰 트레일링 스탑 (하락 {self.trailing_stop_pct}% 하락) 발동: 전량 매도, 거래 종료")
                            self.trade_history.append(trade)
                            self._save_trade_history()
                            self._update_daily_stats(trade["profit_usdt"])
                updated_trades.append(trade)
                ...
                "messages": [
                    ...
                    },
                    {
                        ...
                        "content": (
                            ...
                            f"+5. 위 조건이 충족되면 매수를 진행합니다.\n"
                            f"+6. 잔고의 25%에 레버리지를 곱하여 매수 금액을 결정합니다.\n"
                            f"+7. 목표 수익률에 따라 매도를 분할하여 진행하며, 1차 매도 후 지정 비율(트레일링 스탑, 기본 15%) 하락 시 전량 매도합니다.\n"
...
class BinanceFuturesGUI:
    def __init__(self, root):
        ...
        # Notebook을 사용하여 두 개의 탭(BTC, ETH)으로 분리
...
class SingleCoinGUI:
    def __init__(self, parent, default_symbol):
        ...
        self.trailing_stop = tk.DoubleVar(value=15.0)  # 트레일링 스탑 비율
        self._setup_ui()
    ...
    def start_bot(self):
        try:
            ...
            # 봇 인스턴스 생성 (심볼은 self.symbol에 설정된 값 사용)
