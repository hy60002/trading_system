...
class BinanceFuturesBot:
    ...
    def execute_trade(self):
        ...
        # 마지막 상태 메시지 시간 (1분 간격 메시지 제한용)
        last_status_message_time = datetime.now(self.KST)
        ...
        while self.running:
            try:
                ...
                if last_processed_candle_time is None or latest_candle_time > last_processed_candle_time:
                    self.log(f"🕒 새로운 15분봉 감지: {latest_candle_time.strftime('%Y-%m-%d %H:%M:%S')}")
                        # 추가: 데이터프레임의 마지막 3개 캔들 시간 로깅
                    last_three = df.tail(3)["close_time"].tolist()
                    self.log(f"📊 데이터프레임 최근 3개 캔들 시간: {[t.strftime('%Y-%m-%d %H:%M:%S') for t in last_three]}")
    
                 # 10초 대기 후 데이터 재확인 (API 업데이트 지연 방지)
                    time.sleep(10)
    
                 # 재확인을 위해 데이터 다시 가져오기
                    new_df = self.get_data(remove_incomplete_candle=True)
                    if new_df is not None and len(new_df) >= 14:
                        latest_new_time = new_df["close_time"].iloc[-1]
                    # 새로운 데이터에 최신 캔들이 포함되어 있는지 확인
                        if latest_new_time >= latest_candle_time:
                            df = new_df.copy()
                            latest_candle_time = latest_new_time
                            self.log(f"📊 데이터 재확인 후 최신 캔들 시간: {latest_candle_time.strftime('%Y-%m-%d %H:%M:%S')}")
                ...
                if self.symbol.upper() == "BTCUSDT" and self.entry_signal is not None:
                    time_since_last_message = (current_time - last_status_message_time).total_seconds() / 60
                    if time_since_last_message >= 5:  # 5분마다 메시지 출력
