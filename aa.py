...
# ==============================
CHECK_INTERVAL = 1          # 메인 루프 폴링 간격 (초) → 1초 단위
...
class BinanceFuturesBot:
    def __init__(self, env_path: str, symbol: str = "BTCUSDT", leverage: int = 10, trailing_stop_pct: float = 15.0):
        ...
        self.status_update_interval = 900  # 15분(초) - 본 수정에서는 봉이 완성될 때마다 업데이트
        self.running = False
        self.last_df = None
        ...
        try:
            with self.data_lock:
                ...
                if remove_incomplete_candle:
                    ...
                    if current_time < latest_candle_time:
                        if len(df) >= 4:
                            df = df.iloc[:-1]
                        else:
                            return None
    ...
    def safe_entry(self, entry_type: str, amount_usdt: float):
        """
        entry_type: "LONG" 또는 "SHORT"
        LONG 진입: SIDE_BUY, positionSide="LONG"
        SHORT 진입: SIDE_SELL, positionSide="SHORT"
        주문 수량 계산 로직을 내부에 통합하여 중복 제거.
        """
        try:
    ...
    def safe_exit(self, exit_type: str, quantity: float):
        """
        exit_type: "LONG"이면 롱 청산 (SIDE_SELL), "SHORT"이면 숏 청산 (SIDE_BUY)
        reduceOnly=True 로 주문 실행.
        """
        try:
            coin = self.coin_name()
        ...
        while self.running:
            try:
                ...
                if (self.last_baseline_candle_time is None) or (latest_candle_time > self.last_baseline_candle_time):
                    ...
                    if long_valid:
                        self.entry_signal = long_baseline
                ...
                if self.entry_signal is not None:
                    ...
                    if self.signal_type == "LONG":
                        conditionA = (df["william"].iloc[-1] <= self.entry_signal or df["william"].iloc[-1] <= -90)
                        ...
                        conditionA = (df["william"].iloc[-1] >= self.entry_signal or df["william"].iloc[-1] >= -10)
                        conditionB = (current_close >= previous_close * 1.003)
                        if conditionA or conditionB:
        ...
        with self.trade_lock:
            ...
            for trade in self.active_trades:
                ...
                if trade["position"] == "LONG":
                    ...
                    if trade.get("sell_stage", 0) == 0 and current_price <= entry_price * (2 - first_target):
                        sell_qty = trade["remaining_quantity"] / 2
                    ...
                    elif trade.get("sell_stage", 0) == 1 and current_price <= entry_price * (2 - second_target):
                        sell_qty = trade["remaining_quantity"]
                        sell_order = self.safe_exit("SHORT", sell_qty)
                        if sell_order:
                            trade["second_sell"] = sell_order
                            ...
                            trade["exit_time"] = sell_order["time"]
                            trade["status"] = "CLOSED"
                            trade["profit_pct"] = (entry_price - sell_order["price"]) / entry_price * 100 * self.leverage
    ...
    def update_status(self):
        ...
        while attempt < max_attempts:
            try:
                ...
                coin = self.coin_name()
                # self.last_price가 None이면 "N/A"로 출력
