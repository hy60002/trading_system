...
class TradingConfig:
    ...
    ENTRY_CONDITIONS: Dict[str, Dict[str, Any]] = field(default_factory=lambda: {
        "BTCUSDT": {
            "signal_threshold": 0.25,  # 🔽 완화: 0.3 → 0.25 (더 많은 거래 기회)
            "confidence_required": 45,  # 🔽 완화: 50 → 45
            "timeframe_agreement": 0.25,  # 🔽 완화: 0.3 → 0.25
            "allow_pyramid": True
        },
        "ETHUSDT": {
            "signal_threshold": 0.4,   # 🔽 완화: 0.5 → 0.4
            "confidence_required": 55,  # 🔽 완화: 65 → 55
            "timeframe_agreement": 0.4,  # 🔽 완화: 0.5 → 0.4
            "allow_pyramid": False,
            "btc_correlation_check": True
        },
        "XRPUSDT": {
            "signal_threshold": 0.5,   # 🔽 완화: 0.6 → 0.5
            "confidence_required": 60,  # 🔽 완화: 70 → 60
            "timeframe_agreement": 0.5,  # 🔽 완화: 0.6 → 0.5
            "allow_pyramid": False,
            "btc_correlation_check": True,
    ...
    ML_MIN_PERFORMANCE: float = 0.50  # 🔽 완화: 55% → 50% 정확도 요구사항
    ...
    MIN_NEWS_CONFIDENCE: float = 0.4  # 🔽 완화: 0.5 → 0.4 (더 많은 뉴스 포함)
...
class EnhancedBitgetExchangeManager:
        ...
        while True:
            try:
                if not fallback_mode:
                    ...
                    await asyncio.sleep(5)  # Update every 5 seconds in fallback mode
                ...
                if consecutive_failures >= max_consecutive_failures and not fallback_mode:
                    self.logger.warning(f"⚠️ WebSocket 연속 실패 {consecutive_failures}회 - 대체 모드 활성화")
                    fallback_mode = True
                    consecutive_failures = 0  # Reset counter
                    await asyncio.sleep(10)  # Brief pause before switching modes
                    continue
                ...
                self.logger.info(f"🕰️ {wait_time:.1f}초 후 WebSocket 재연결 시도...")
    ...
    async def _connect_websocket(self):
        ...
        ws_urls = [
            ...
            "wss://ws.bitgetapi.com/mix/v1/stream", 
            "wss://ws.bitget.com/spot/v1/stream"  # Alternative endpoint
        ]
        ...
        for i, ws_url in enumerate(ws_urls):
            try:
                ...
                async with websockets.connect(
                    ...
                    user_agent_header="BitgetAPI/3.0",  # Add user agent
                    extra_headers={
                ...
                self.logger.warning(f"⚠️ WebSocket URL {i+1} 실패: {type(e).__name__} - {str(e)[:50]}")
    ...
    async def _handle_websocket_messages(self, websocket):
        ...
        heartbeat_timeout = 90  # Reduced to 1.5 minutes
        message_count = 0
...
평균 켈리 지수: {np.mean([d['kelly_fraction'] for d in daily_data if d['kelly_fraction'] > 0]):.3f}
...
class EnhancedNewsSentimentAnalyzer:
    ...
    async def analyze_sentiment(self, symbol: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Emergency check - Found: {has_emergency}, Severity: {emergency_severity:.1f}")
            ...
            if self.openai_client:
                try:
                    ...
                    self.logger.info(f"GPT sentiment analysis successful for {symbol}")
            ...
            if analysis is None or not analysis.get('sentiment'):
                ...
                self.logger.info(f"Using enhanced basic sentiment analysis for {symbol}")
            ...
            if has_emergency:
                ...
                if emergency_severity >= 3.0:
                    ...
                    analysis['action_required'] = 'reduce_positions' 
                    analysis['urgency'] = '높음'
                else:
                ...
                self.logger.warning(f"⚠️ Emergency action: {analysis['action_required']} (severity: {emergency_severity:.1f})")
            ...
            for keyword, severity in self.emergency_keywords.items():
                if keyword in text:
                    ...
                    if actual_severity >= 2.5:  # Only consider critical as true emergency
                        self.logger.warning(f"🚨 Critical keyword detected (severity: {actual_severity:.1f}): {item['title']}")
                        emergency_found = True
                    else:
                        self.logger.info(f"⚠️ Caution keyword detected (severity: {actual_severity:.1f}): {item['title']}")
                        
        return emergency_found, max_severity
    
    def _assess_content_quality(self, news_item: Dict) -> float:
        ...
        if len(title) > 10:
            quality_score += 0.1
        if len(title) < 100:  # Not too long (likely clickbait)
            quality_score += 0.1
        
        # Content length assessment
        if len(summary) > 50:  # Has meaningful summary
            quality_score += 0.15
        if len(summary) > 200:  # Detailed content
            quality_score += 0.1
            
        # Language quality indicators
        if not any(char in title.lower() for char in ['🚀', '💰', '⚡', '🔥']):  # No excessive emojis
            quality_score += 0.05
        
        # Professional writing indicators
        professional_terms = ['analysis', 'report', 'according to', 'data shows', 'research', 'study']
        if any(term in title.lower() or term in summary.lower() for term in professional_terms):
            quality_score += 0.1
            
        return min(1.0, quality_score)
    
    def _is_spam_content(self, news_item: Dict) -> bool:
        ...
        # Spam indicators
        spam_patterns = [
            'click here', 'limited time', 'urgent', 'act now', 'don\'t miss',
            'guaranteed', 'risk free', 'amazing opportunity', 'get rich', 
            'millionaire', 'secret method', 'insider tip'
        ]
        
        # Excessive capitalization
        if len([c for c in title if c.isupper()]) > len(title) * 0.3:
            return True
    ...
    def _filter_reliable_news(self, news_items: List[Dict]) -> List[Dict]:
        """Filter news by reliability and confidence score with enhanced quality checks"""
        filtered_news = []
        ...
        for item in news_items:
            ...
            # Enhanced filtering with source reliability and content quality
            source_reliability = item.get('source_reliability', 0.5)
            combined_score = (confidence_score * 0.7) + (source_reliability * 0.3)
            
            # Apply content quality filters
            content_quality = self._assess_content_quality(item)
            final_score = (combined_score * 0.8) + (content_quality * 0.2)
            
            # Only include news with sufficient score and no spam indicators
            if final_score >= self.config.MIN_NEWS_CONFIDENCE and not self._is_spam_content(item):
                item['confidence_score'] = confidence_score
                item['combined_score'] = combined_score
                item['content_quality'] = content_quality
                item['final_score'] = final_score
                item['source_reliability'] = source_reliability
                filtered_news.append(item)
            else:
                reason = "low score" if final_score < self.config.MIN_NEWS_CONFIDENCE else "spam detected"
                self.logger.debug(f"Filtered out news ({reason}): {item['title'][:50]}...")
        
        # Sort by final score and return top quality news
        filtered_news.sort(key=lambda x: x.get('final_score', 0), reverse=True)
        self.logger.info(f"📊 News filtering: {len(filtered_news)}/{len(news_items)} articles passed quality filters")
        return filtered_news[:10]  # Return top 10 highest quality news
...
Consider the confidence scores when weighing news importance."""
    ...
    def _get_enhanced_basic_analysis(self, news_items: List[Dict], has_emergency: bool, symbol: str) -> Dict[str, Any]:
        ...
        for item in news_items:
            ...
            for keyword, weight in positive_keywords.items():
                if keyword in text:
                    ...
                    if weight >= 2.0:  # High impact keywords
                        positive_factors.append(f"{keyword.title()} detected")
            ...
            for keyword, weight in negative_keywords.items():
                if keyword in text:
                    ...
                    item_confidence += 0.1
                    if weight <= -2.0:  # High impact keywords
                        negative_factors.append(f"{keyword.title()} detected")
        ...
        if overall_sentiment > 0.3:
            summary = f"Positive sentiment detected with {pos_count} bullish signals"
        elif overall_sentiment < -0.3:
            summary = f"Negative sentiment detected with {neg_count} bearish signals"
        else:
            summary = f"Neutral sentiment with mixed signals ({pos_count} positive, {neg_count} negative)"
...
"""
    ...
    def _calculate_ml_health(self) -> float:
        ...
        try:
            ...
            performances = self.db.get_model_performance()
            if performances:
                avg_accuracy = np.mean([p.get('accuracy', 0.5) for p in performances.values()])
                health += min(0.3, (avg_accuracy - 0.5) * 0.6)
                
            # Check recent predictions
            recent_predictions = getattr(self.db, '_predictions_cache', [])
    ...
    def _calculate_news_health(self) -> float:
        ...
        try:
            ...
            if not hasattr(self, '_last_news_fetch'):
                self._last_news_fetch = time.time()
                
            time_since_news = time.time() - self._last_news_fetch
            if time_since_news < 3600:  # Less than 1 hour
                health += 0.3
    ...
    def _get_daily_trade_count(self) -> int:
        ...
        try:
            ...
            # Simple implementation - would need actual database query
    ...
    def _calculate_error_rate(self) -> float:
        ...
        return getattr(self, '_error_rate', 0.0)
        
    def _get_performance_summary(self) -> Dict:
        """Get performance summary"""
        try:
            perf = self.db.get_daily_performance()
            return {
