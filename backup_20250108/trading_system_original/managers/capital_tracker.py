...
Monitors 33% allocation limit and provides real-time position tracking
"""
...
class CapitalTracker:
    ...
    async def update_snapshot(self) -> CapitalSnapshot:
        ...
        try:
            ...
            within_limit = allocation_percentage <= self.config.MAX_TOTAL_ALLOCATION
            
            # Create snapshot
            self.current_snapshot = CapitalSnapshot(
                total_balance=total_balance,
                used_capital=used_capital,
                available_capital=total_balance * self.config.MAX_TOTAL_ALLOCATION - used_capital,
            ...
            if self.update_count % 10 == 0:  # Every 10 updates (5 minutes)
                self.logger.info(
                    ...
                    f"({used_capital:,.0f}/{total_balance * self.config.MAX_TOTAL_ALLOCATION:,.0f})"
                )
    ...
    async def _get_total_balance(self) -> float:
        ...
        try:
            ...
            if balance_data:
                return float(balance_data.get('total_balance', 10000))
            else:
                # Default fallback balance for testing
                self.logger.warning("⚠️ 잔고 데이터 없음 - 기본값 사용")
                return 10000.0
...
계속 모니터링 중입니다."""
    ...
    def can_open_position(self, symbol: str, estimated_cost: float) -> Tuple[bool, str, Dict]:
        """Check if new position can be opened within 33% limit"""
        if not self.current_snapshot:
        ...
        checks = {
            'within_allocation_limit': new_allocation <= self.config.MAX_TOTAL_ALLOCATION,
    ...
    def _check_symbol_weight(self, symbol: str, additional_cost: float) -> bool:
        ...
        total_target_allocation = self.current_snapshot.total_balance * self.config.MAX_TOTAL_ALLOCATION
...
async def get_available_capital_for_symbol(tracker: CapitalTracker, symbol: str) -> float:
    ...
    total_target_allocation = tracker.current_snapshot.total_balance * tracker.config.MAX_TOTAL_ALLOCATION
    max_symbol_allocation = total_target_allocation * target_weight
