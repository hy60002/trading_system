 ...
 * @description 동적 할당 한도 실시간 모니터링 컴포넌트
 */
...
export class CapitalTracker extends BaseComponent {
                    ...
                    <div class="allocation-chart">
                        <div class="progress-ring">
                            ...
                            <div class="progress-text">
                                ...
                                <div class="limit" id="allocation-limit">/ --%</div>
                            </div>
                        </div>
                    </div>
    ...
    /**
     * Display elements 업데이트
     */
    updateDisplayElements() {
        // 동적 한도 표시 업데이트
        const limitElement = this.container.querySelector('#allocation-limit');
        if (limitElement && this.data.allocationLimit) {
            limitElement.textContent = `/ ${(this.data.allocationLimit * 100).toFixed(0)}%`;
        }
    }
    ...
    getStrokeOffset() {
        ...
        const limit = this.data.allocationLimit || 0.33;
        const progress = Math.min(this.data.allocationPercentage / limit, 1); // 동적 한도 기준
