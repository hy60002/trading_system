...
class EnhancedDatabaseManager:
    ...
    def update_daily_performance(self, date: str, performance_data: Dict):
        ...
        try:
            if self.performance_dao:
                return self.performance_dao.update_daily_performance(date, performance_data)
            else:
                self.logger.warning("PerformanceDAO가 사용할 수 없어 일일 성과를 업데이트할 수 없습니다")
        except Exception as e:
            ...
            with self._get_connection() as conn:
                ...
                cursor.execute("""
                    ...
                    (symbol, timeframe, price, prediction, confidence, direction, 
                     ...
                     regime, indicators_snapshot, timestamp)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    ...
                    json.dumps(prediction_data.get('indicators_snapshot', {})),
