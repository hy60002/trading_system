...
class TradingConfig:
    ...
    CAPITAL_ALLOCATION_LIMIT: float = 1.0   # 자본 할당 한도 (전체 잔고 100% 사용 가능)
    FALLBACK_BALANCE: float = 1000.0  # API 실패 시 사용할 기본 잔고 (기존 10000에서 보수적으로 변경)
    ENABLE_DYNAMIC_BALANCE: bool = True  # 실제 API 잔고 사용 여부 (False면 FALLBACK_BALANCE 사용)
    BALANCE_CACHE_TIMEOUT: int = 30  # 잔고 조회 캐시 시간 (초)
    ...
    NEWS_WEIGHT: float = 0.5  # 50% weight for news sentiment
    
    # Enhanced Features from trading_system2
    # ATR 기반 TP/SL 설정 (trading_system2 호환)
    ATR_SETTINGS_SIMPLE: Dict[str, Dict[str, float]] = field(default_factory=lambda: {
        "BTCUSDT": {"period": 14, "stop_multiplier": 2.0, "profit_multiplier": 3.0},
        "ETHUSDT": {"period": 14, "stop_multiplier": 2.5, "profit_multiplier": 3.5},
    })
    
    # 주문 재시도 횟수 (trading_system2 호환)
    ORDER_RETRY: int = 3
