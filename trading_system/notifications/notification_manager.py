...
class NotificationManager:
    ...
    async def send_notification(self, message: str, priority: str = 'normal', 
                              channels: list = None, metadata: Dict = None):
        """Send notification with priority and multi-channel support"""
        # 기본 채널 설정 (하위 호환성 위해 기존 단일 채널도 지원)
        if channels is None:
            channels = ['telegram']  # 기본값
        elif isinstance(channels, str):
            channels = [channels]  # 단일 문자열을 리스트로 변환
        
        # 사용 가능한 채널 목록
        available_channels = ['telegram', 'slack', 'email']
        valid_channels = [ch for ch in channels if ch in available_channels]
        
        if not valid_channels:
            self.logger.warning(f"⚠️ 유효한 채널 없음: {channels}, telegram 사용")
        ...
        if priority != 'emergency' and not is_trade_notification:
            if message_hash in self.last_message_time:
                if current_time - self.last_message_time[message_hash] < self.min_interval:
                    ...
                    return {'success': False, 'reason': 'duplicate_blocked', 'channels': {}}
        ...
        for channel in valid_channels:
            try:
                # Create message object for each channel
                msg_obj = {
                    'content': message,
                    'channel': channel,
                    ...
                    'retry_count': 0,
                    'max_retries': 3 if is_trade_notification else 1
                }
                
                # 채널별 전송 실행
                result = await self._send_to_channel(channel, msg_obj)
                ...
                self.logger.info(f"📤 {channel} 채널 전송: {'성공' if result else '실패'}")
                ...
                self.logger.error(f"❌ {channel} 채널 전송 오류: {e}")
        
        # 전체 결과 반환
        overall_success = success_count > 0  # 하나라도 성공하면 성공으로 간주
        
        self.logger.info(f"📊 다중 채널 전송 완료: {success_count}/{len(valid_channels)} 성공")
    ...
    async def _send_to_channel(self, channel: str, msg_obj: dict) -> bool:
        """채널별 전송 메서드"""
        try:
            if channel == 'telegram':
                return await self._send_to_telegram(msg_obj)
            elif channel == 'slack':
                return await self._send_to_slack(msg_obj)
            elif channel == 'email':
                return await self._send_to_email(msg_obj)
            else:
                self.logger.error(f"❌ 지원되지 않는 채널: {channel}")
                return False
        except Exception as e:
            self.logger.error(f"❌ {channel} 채널 전송 오류: {e}")
            return False
    
    async def _send_to_telegram(self, msg_obj: dict) -> bool:
        """기존 텔레그램 전송 로직 활용"""
        try:
            # 기존 텔레그램 전송 로직 호출
            if hasattr(self, '_send_telegram_message'):
                return await self._send_telegram_message(msg_obj['content'])
            else:
                self.logger.info(f"📱 [TELEGRAM] {msg_obj['content']}")
                return True  # 임시 성공 반환
        except Exception as e:
            self.logger.error(f"❌ 텔레그램 전송 실패: {e}")
            return False
    
    async def _send_to_slack(self, msg_obj: dict) -> bool:
        """슬랙 전송 로직 (향후 구현)"""
        try:
            self.logger.info(f"💬 [SLACK] {msg_obj['content']}")
            # TODO: 실제 Slack API 연동 구현
            return True  # 임시 성공 반환
        except Exception as e:
            self.logger.error(f"❌ 슬랙 전송 실패: {e}")
            return False
    
    async def _send_to_email(self, msg_obj: dict) -> bool:
        """이메일 전송 로직 (향후 구현)"""
        try:
            self.logger.info(f"📧 [EMAIL] {msg_obj['content']}")
            # TODO: 실제 Email API 연동 구현
            return True  # 임시 성공 반환
        except Exception as e:
